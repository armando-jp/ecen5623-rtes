...
void print_scheduler(void)
{
   ...
   // Returns the contention scope attribute of the thread attributes object
   // referred to by attr [main_attr] in the buffer pointed to by scope [scope].
   //
   // The contention scope of a user thread defines how it is mapped to a kernel
   // thread. 
   //
   // PTHREAD_SCOPE_SYSTEM: System contention scope, sometimes called global
   // contention scope. Specifies that the thread will be scheduled against all
   // other threads in the system and is directly mapped to one kernel thread.
...
int main (int argc, char *argv[])
{
   ...
   // set the scheduling policy and the priority for the main thread.
   ...
   {
      // config thread for fifo scheduling
       ...
       // config priority to one less than the max allowed for fifo scheduling. 
