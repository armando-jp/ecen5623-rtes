...
uint32_t sequencer_delay_time = pdUS_TO_TICKS(333);
...
void vApplicationTickHook(void)
{
    ...
    if(seqCnt < sequence_periods)
    {
        ...
        if((xTaskGetTickCount()%sequencer_delay_time) == 0)
        {
            ...
            if((seqCnt % 30) == 0) xSemaphoreGiveFromISR(t7_sem, NULL);
...
{
    static uint32_t current_time_val = 0;
    static uint32_t start_time_val = 0;
    ...
    while(!abortS1)
    {
        ...
        {
            start_time_val = pdTICKS_TO_MS(xTaskGetTickCount());
            ...
            if(xSemaphoreTake(print_sem, portMAX_DELAY) == pdTRUE)
            {
                ...
                UARTprintf("service_1 release @ sec = %d, msec = %d! c = (ms)%d\r\n", current_time_val/1000, current_time_val%1000, current_time_val-start_time_val);
                xSemaphoreGive(print_sem);
            }
        }
    }
    ...
    while(!abortS2)
    {
        if(xSemaphoreTake(t2_sem, portMAX_DELAY) == pdTRUE)
        {
            ...
            if(xSemaphoreTake(print_sem, portMAX_DELAY) == pdTRUE)
            {
                ...
                UARTprintf("service_2 release @ sec = %d, msec = %d! c = (ms)%d\r\n", current_time_val/1000, current_time_val%1000, current_time_val-start_time_val);
                xSemaphoreGive(print_sem);
            }
        }
    }
    ...
    while(!abortS3)
    {
        if(xSemaphoreTake(t3_sem, portMAX_DELAY) == pdTRUE)
        {
            ...
            if(xSemaphoreTake(print_sem, portMAX_DELAY) == pdTRUE)
            {
                ...
                UARTprintf("service_3 release @ sec = %d, msec = %d! c = (ms)%d\r\n", current_time_val/1000, current_time_val%1000, current_time_val-start_time_val);
                xSemaphoreGive(print_sem);
            }
        }
    }
    ...
    while(!abortS4)
    {
        if(xSemaphoreTake(t4_sem, portMAX_DELAY) == pdTRUE)
        {
            ...
            if(xSemaphoreTake(print_sem, portMAX_DELAY) == pdTRUE)
            {
                ...
                UARTprintf("service_4 release @ sec = %d, msec = %d! c = (ms)%d\r\n", current_time_val/1000, current_time_val%1000, current_time_val-start_time_val);
                xSemaphoreGive(print_sem);
            }
        }
    }
    ...
    while(!abortS5)
    {
        if(xSemaphoreTake(t5_sem, portMAX_DELAY) == pdTRUE)
        {
            ...
            if(xSemaphoreTake(print_sem, portMAX_DELAY) == pdTRUE)
            {
                ...
                UARTprintf("service_5 release @ sec = %d, msec = %d! c = (ms)%d\r\n", current_time_val/1000, current_time_val%1000, current_time_val-start_time_val);
                xSemaphoreGive(print_sem);
            }
        }
    }
    ...
    while(!abortS6)
    {
        if(xSemaphoreTake(t6_sem, portMAX_DELAY) == pdTRUE)
        {
            ...
            if(xSemaphoreTake(print_sem, portMAX_DELAY) == pdTRUE)
            {
                ...
                UARTprintf("service_6 release @ sec = %d, msec = %d! c = (ms)%d\r\n", current_time_val/1000, current_time_val%1000, current_time_val-start_time_val);
                xSemaphoreGive(print_sem);
            }
        }
    }
    ...
    while(!abortS7)
    {
        if(xSemaphoreTake(t7_sem, portMAX_DELAY) == pdTRUE)
        {
            ...
            if(xSemaphoreTake(print_sem, portMAX_DELAY) == pdTRUE)
            {
                ...
                UARTprintf("service_7 release @ sec = %d, msec = %d! c = (ms)%d\r\n", current_time_val/1000, current_time_val%1000, current_time_val-start_time_val);
                xSemaphoreGive(print_sem);
            }
        }
    }
