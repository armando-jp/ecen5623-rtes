...
uint32_t sequencer_delay_time = pdMS_TO_TICKS(16);
    ...
    if(seqCnt < sequence_periods)
    {
        ...
        if((xTaskGetTickCount()%sequencer_delay_time) == 0)
        {
            ...
            // Servcie_1 = RT_MAX-1 @ 3 Hz
             if((seqCnt % 2) == 0) xSemaphoreGiveFromISR(t1_sem, NULL);

            // Service_2 = RT_MAX-2 @ 1 Hz
            if((seqCnt % 6) == 0) xSemaphoreGiveFromISR(t2_sem, NULL);

            // Service_3 = RT_MAX-3 @ 0.5 Hz
            if((seqCnt % 12) == 0) xSemaphoreGiveFromISR(t3_sem, NULL);

            // Service_4 = RT_MAX-2 @ 1 Hz
            if((seqCnt % 6) == 0) xSemaphoreGiveFromISR(t4_sem, NULL);

            // Service_5 = RT_MAX-3 @ 0.5 Hz
            if((seqCnt % 12) == 0) xSemaphoreGiveFromISR(t5_sem, NULL);

            // Service_6 = RT_MAX-2 @ 1 Hz
            if((seqCnt % 6) == 0) xSemaphoreGiveFromISR(t6_sem, NULL);

            // Service_7 = RT_MIN   0.1 Hz
            if((seqCnt % 60) == 0) xSemaphoreGiveFromISR(t7_sem, NULL);
